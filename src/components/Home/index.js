...
class Home extends Component {
  ...
  onClickButton = id => {
    ...
    const img = imagesList[randomNo]
    if (randomImage === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
      }))
      this.setState({
        randomImage: img.imageUrl,
      })
    } else {
      clearInterval(this.timerId)
      this.setState({
  ...
  render() {
    const {score, count, randomImage, scoreCard} = this.state
